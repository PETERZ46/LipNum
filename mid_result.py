...
SOURCE_PATH = r'C:\Users\zhengqi\Desktop\ghn'
SOURCE_EXTS = r'*.mp4'
...
for filepath in find_files(SOURCE_PATH, SOURCE_EXTS):
    ...
    mouth_frames_mean_re = np.reshape(mouth_frames_mean, newshape=[24, ])
    data.append(mouth_frames_mean_re)
...
dataset.to_csv(r'dataset\dataset_ghn.csv')
name = ['state']
labelset = pd.DataFrame(columns=name, data=label)
labelset.to_csv(r'dataset\labelset_ghn.csv')
